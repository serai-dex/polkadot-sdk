...
mod tests {
	...
	use substrate_test_runtime_client::ExtendHostFunctions;

	fn executor() -> WasmExecutor<substrate_test_runtime_client::ExtendHostFunctions> {
	...
	where
		F: Fn(&Path, &[u8], &WasmExecutor<ExtendHostFunctions>),
	{
		let exec = executor();
