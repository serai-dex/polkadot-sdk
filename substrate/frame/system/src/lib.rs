...
use frame_support::{
	dispatch::{
		...
		DispatchResultWithPostInfo, PerDispatchClass,
	},
...
// Hash used when building the genesis config,
fn build_genesis_hash<T: AsMut<[u8]> + Default>() -> T {
	let mut h = T::default();
	// 0xfe. Close to "ge"nesis, not 0xff, Rust.
	h.as_mut().iter_mut().for_each(|byte| *byte = 0xfe);
	h
}
