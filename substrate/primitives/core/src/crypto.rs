...
pub trait Ss58Codec: Sized + AsMut<[u8]> + AsRef<[u8]> + ByteArray {
	...
	fn to_ss58check_with_version(&self, version: Ss58AddressFormat) -> String {
		...
		let mut v = match ident {
			...
			_ => unreachable!("masked out the upper two bits"),
		};
...
impl sp_std::str::FromStr for SecretUri {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		Ok(Self {
			phrase: SecretString::new(phrase.to_string()),
			password: cap.pass.map(|v| SecretString::new(v.to_string())),
...
pub trait Pair: CryptoType + Sized {
	...
	fn generate_with_phrase(password: Option<&str>) -> (Self, String, Self::Seed) {
		let mnemonic = Mnemonic::generate(12).expect("Mnemonic generation always works");
	...
	fn from_phrase(
		...
		let mut big_seed = mnemonic.to_seed(password.unwrap_or(""));
		let mut seed = Self::Seed::default();
	...
	fn from_seed(seed: &Self::Seed) -> Self {
		Self::from_seed_slice(seed.as_ref()).expect("seed has valid length")
	}
	...
	fn from_string_with_seed(
		...
		let (root, seed) = if let Some(stripped) = phrase.0.strip_prefix("0x") {
